...
class StitchSchemataIO(IO):
    """
    Output style for StitchSchemata.
    """
    ...
    # ------------------------------------------------------------------------------------------------------------------
    def log_notice(self, message: Union[str, Iterable[str]]) -> None:
        """
        Logs a message only when logging level is notice/normal.

        :param message: The message or messages.
        """
        if self._output.verbosity.value >= Verbosity.NORMAL.value:
            self.text(message)
